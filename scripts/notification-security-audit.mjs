...
for (const forbidden of [
  ...
  if (client.includes(forbidden)) throw new Error(`Browser notification client still accesses Supabase directly: ${forbidden}`);
}
...
if (legacy.includes('export async function sbGetNotifications')) {
  console.log('legacy notification implementation retained only for compatibility archive; active client must use js/services/notifications.js');
}
