...
} from '../js/utils/controllerLoader.js';
...
// 3. File controller benar-benar dapat dimuat tanpa module resolution / syntax error
{
  ...
  assert.match(appMainCoreContent, /import\s+['"]\.\/app-main-controller\.js['"]/, 'app-main-core.js must import app-main-controller.js');

  const tokoSayaCoreContent = fs.readFileSync(path.join(ROOT_DIR, 'js/toko-saya-core.js'), 'utf8');
  assert.match(tokoSayaCoreContent, /import\s+['"]\.\/toko-saya-controller\.js['"]/, 'toko-saya-core.js must import toko-saya-controller.js');
...
// 5. Bootstrap tetap menangani rejection dengan .catch()
{
  ...
  assert.match(appMainCode, /\[App bootstrap\] Gagal memuat controller utama\./, 'js/app-main.js must log error on load failure');
  ...
  assert.match(tokoSayaCode, /\[Toko Saya bootstrap\] Gagal memuat controller\./, 'js/toko-saya-legacy.js must log error on load failure');
...
// 6. Tidak terjadi duplicate initialization ketika bootstrap dipanggil lebih dari sekali
{
  ...
  const mockControllerUrl = 'data:text/javascript,let initCount = 0; export function init() { return ++initCount; } export const id = Math.random();';
  ...
  console.log('✓ 6. No duplicate initialization: identical Promise and singleton module instance shared across invocations');
}
