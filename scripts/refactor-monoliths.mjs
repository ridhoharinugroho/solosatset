...
function compact(src) {
  ...
  while (i < src.length) {
    ...
    if (c === "'" || c === '"' || c === "`") {
      if (pending && id(out.at(-1)) && id(c)) out += " ";
      pending = false;
      const q = c;
      out += c;
      i++;
    ...
    if (c === "/") {
      ...
      const prev = out[j] || "";
      const wm = out.slice(0, j + 1).match(/([A-Za-z_$][\w$]*)$/);
      const word = wm ? wm[1] : "";
      const regexStart = !prev || "=([{,:;!&|?+\-*%^~<>".includes(prev) || regexWords.has(word);
      if (regexStart) {
        if (pending && regexWords.has(word)) out += " ";
        pending = false;
        out += "/";
        i++;
        let cls = false;
        ...
        while (i < src.length && /[A-Za-z]/.test(src[i])) out += src[i++];
        continue;
      }
    }
    if (pending) {
