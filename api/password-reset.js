...
function safeEqualHex(left, right) {
  try { const a = Buffer.from(String(left), 'hex'), b = Buffer.from(String(right), 'hex'); return a.length === b.length && crypto.timingSafeEqual(a, b); } catch { return false; }
}
...
function allowRequest(req, email) {
  ...
  recent.push(now); requestLimiter.set(key, recent); return true;
}
async function getSmtpConfig(supabase) {
  const { data: row, error } = await supabase.from('app_smtp_config').select('settings_json').eq('id', 'config').maybeSingle();
  if (error) throw new Error('SMTP configuration lookup failed.');
  const parsed = row?.settings_json ? (typeof row.settings_json === 'string' ? JSON.parse(row.settings_json) : row.settings_json) : {};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'request') {
      ...
      try { await sendResetEmail(supabase, { email, userName: user.name || user.store_name, resetCode }); }
