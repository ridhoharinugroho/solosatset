...
function hashPassword(password) {
  ...
  const hash = crypto.scryptSync(String(password), salt, 64, { N: n, r, p, maxmem: 32 * 1024 * 1024 });
...
function allowRequest(req, email) {
  const key = getRequestKey(req, email), now = Date.now();
  const recent = (requestLimiter.get(key) || []).filter((ts) => now - ts < REQUEST_WINDOW_MS);
...
function getSmtpConfig() {
  ...
  const fromName = String(process.env.SMTP_FROM_NAME || 'Pusat Jual Beli Solo Raya').trim();
  const fromEmail = String(process.env.SMTP_FROM || user).trim();
  if (!host || !Number.isInteger(port) || port < 1 || port > 65535 || !user || !pass || !fromEmail) {
  ...
  try {
    ...
    if (action === 'request') {
      ...
      if (!user || String(user.status || 'active').toLowerCase() === 'deleted' || user.deleted_at) {
        return res.status(200).json({ success: true, message: 'Jika akun ditemukan dan aktif, instruksi pemulihan telah diproses.' });
      }

      const resetCode = String(crypto.randomInt(100000, 1000000));
      const createdAt = new Date().toISOString();
      const expiresAt = new Date(Date.now() + OTP_TTL_MS).toISOString();
      const { error: storeError } = await supabase.from('otp_sessions').upsert({
        email,
      ...
      try {
        await sendResetEmail({ email, userName: user.name || user.store_name, resetCode });
      } catch (mailError) {
    ...
    if (action === 'reset') {
      ...
      if (newPassword.trim().length < 8) return res.status(400).json({ success: false, error: 'Password baru minimal 8 karakter.' });

      const result = await supabase.rpc('consume_otp_and_reset_password', {
        p_email: email,
        p_code_hash: hashOtp(otpCode),
        p_password_hash: hashPassword(newPassword)
      });
      if (result.error) {
        console.error('[Password Reset RPC]', result.error.message);
        return res.status(500).json({ success: false, error: 'Gagal memproses reset password. Silakan coba lagi.' });
      }

      const status = String(result.data || '').toLowerCase();
      if (status === 'success') return res.status(200).json({ success: true, email });
      if (status === 'locked') return res.status(429).json({ success: false, error: 'Batas percobaan kode verifikasi telah tercapai. Silakan minta kode baru.' });
      if (status === 'expired') return res.status(400).json({ success: false, error: 'Kode verifikasi telah kadaluarsa. Silakan minta kode baru.' });
      if (status === 'account_missing') return res.status(400).json({ success: false, error: 'Akun tidak tersedia atau sudah dinonaktifkan.' });
      return res.status(400).json({ success: false, error: 'Kode verifikasi yang Anda masukkan salah atau kadaluarsa.' });
    }
