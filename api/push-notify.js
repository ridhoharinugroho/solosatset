...
async function handleUserNotificationAction(req, res, action) {
  ...
  if (!supabase)
    return res.status(503).json({ success: false, error: "Notification service is not configured on the server." });
  ...
  if (action === "broadcast_bu_notification") {
    ...
    if (!listing || String(listing.seller_id || "") !== userId) {
      return res
        .status(403)
        .json({ success: false, userCount: 0, error: "Anda tidak berwenang mengirim notifikasi untuk produk ini." });
    }
    if (!listing.is_bu) {
    ...
    const title = rawTitle
      ? rawTitle.includes("BUTUH UANG")
        ? rawTitle
        : `🔥 BUTUH UANG CEPAT: ${rawTitle}`
      : "🔥 IKLAN BUTUH UANG CEPAT (BU) TERBARU!";
    const message = clean(
      details.message ||
        `Ada iklan butuh uang cepat (BU) untuk kategori ${categoryId} yang Anda minati! Cek sekarang sebelum keduluan.`,
      2000,
    );
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET" && requestedAction === "list_notifications") {
    try {
      return await handleUserNotificationAction(req, res, requestedAction);
    } catch (error) {
  ...
  if (req.method === "GET") {
    if (!VAPID_PUBLIC_KEY)
      return res
        .status(503)
        .json({ success: false, error: "Push notification service is not configured on the server." });
    return res
      .status(200)
      .json({
  ...
  if (["mark_notification_read", "mark_all_notifications_read", "broadcast_bu_notification"].includes(action)) {
    try {
      return await handleUserNotificationAction(req, res, action);
    } catch (error) {
  ...
  try {
    ...
    const cleanIds = Array.isArray(itemPayload.targetUserIds)
      ? itemPayload.targetUserIds
          ...
          .filter(Boolean)
          .map((u) => String(u).trim().slice(0, 128))
          .filter(Boolean)
          .slice(0, 500)
      : itemPayload.targetUserId
        ? [String(itemPayload.targetUserId).trim().slice(0, 128)]
        : [];
    const targetEmail = itemPayload.targetEmail
      ? String(itemPayload.targetEmail).trim().toLowerCase().slice(0, 320)
      : null;
    ...
    const allSubs = Array.isArray(subscriptions)
      ? subscriptions.filter((s) => s?.endpoint && s?.p256dh && s?.auth)
      : [];
    if (hasTarget && allSubs.length === 0)
      return res
        .status(200)
        .json({
          success: true,
          ...
          failedCount: 0,
          totalTargets: 0,
          message: "Tidak ada target pengguna yang terdaftar.",
        });
    if (!hasTarget && allSubs.length === 0)
      return res
        .status(200)
        .json({
          ...
          failedCount: 0,
          totalTargets: 0,
          message: "Belum ada perangkat pengguna yang terdaftar.",
        });
