...
export default async function handler(req, res) {
  ...
  try {
    ...
    let body = req.body;
    if (typeof body === 'string') { try { body = JSON.parse(body); } catch { return res.status(400).json({ success: false, error: 'Invalid JSON body.' }); } }
    const itemPayload = Array.isArray(body) ? (body[0] || {}) : (body || {});
    const title = String(itemPayload.title || '📢 Pusat Jual Beli Solo Raya').slice(0, 200);
    const message = String(itemPayload.body || itemPayload.message || 'Ada pembaruan sistem dan barang seken terbaru di Solo Raya! Buka aplikasi sekarang.').slice(0, 2000);
    const url = String(itemPayload.url || '/').slice(0, 1000);
    const icon = String(itemPayload.icon || itemPayload.image || '/assets/img/app-logo.png?v=2.1').slice(0, 500);
    const badge = String(itemPayload.badge || '/assets/img/app-logo.png?v=2.1').slice(0, 500);
    const tag = String(itemPayload.tag || 'solosatset-update').slice(0, 100);
