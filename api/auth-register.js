...
async function sendWelcomeEmail(user) { const smtp = getSmtpSettings(); if (!smtp || !user.email) return; try { const transporter = nodemailer.createTransport({ host: smtp.host, port: smtp.port, secure: smtp.secure, auth: { user: smtp.user, pass: smtp.pass }, connectionTimeout: 15000, tls: { rejectUnauthorized: true } }); await transporter.sendMail({ from: `"${smtp.fromName}" <${smtp.fromEmail}>`, to: user.email, subject: 'Selamat datang di Pusat Jual Beli Solo Raya', text: `Halo ${user.name || 'Penjual'}, akun Anda berhasil dibuat. Selamat mulai berjualan di Pusat Jual Beli Solo Raya.`, html: `<p>Halo ${user.name || 'Penjual'},</p><p>Akun Anda berhasil dibuat. Selamat mulai berjualan di Pusat Jual Beli Solo Raya.</p>` }); } catch (error) { console.warn('[Auth Register SMTP]', error.message); } }
