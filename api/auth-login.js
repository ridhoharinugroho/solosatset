...
async function findUser(supabase, identifier) {
  const clean = normalizeIdentifier(identifier), email = normalizeEmail(clean), phone = normalizePhone(clean);
  const queries = [supabase.from('users').select(USER_FIELDS).eq('email', email).limit(1), supabase.from('users').select(USER_FIELDS).eq('name', clean).limit(1), supabase.from('users').select(USER_FIELDS).eq('store_name', clean).limit(1)];
  if (phone.length >= 7) { queries.push(supabase.from('users').select(USER_FIELDS).eq('phone', clean).limit(1)); queries.push(supabase.from('users').select(USER_FIELDS).eq('phone', phone).limit(1)); }
  const results = await Promise.all(queries), candidates = [];
  for (const result of results) { if (result.error) throw result.error; if (Array.isArray(result.data) && result.data[0]) candidates.push(result.data[0]); }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (needsLegacyMigration) {
      ...
      const { error: upgradeError } = await supabase.from('users').update({ password_hash: upgradedHash, password: null, updated_at: new Date().toISOString() }).eq('id', user.id);
