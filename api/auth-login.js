...
function parseScryptHash(value) {
  ...
  if (parts.length === 6 && parts[0] === "scrypt") {
    ...
    const n = Number(nRaw),
      r = Number(rRaw),
      p = Number(pRaw);
    if (!Number.isSafeInteger(n) || !Number.isSafeInteger(r) || !Number.isSafeInteger(p) || !saltText || !hashText)
      return null;
    try {
...
function verifyPassword(password, encoded) {
  ...
  try {
    const hash = crypto.scryptSync(String(password), parsed.salt, parsed.hash.length, {
      N: parsed.n,
      r: parsed.r,
      p: parsed.p,
      maxmem: 32 * 1024 * 1024,
...
function getRateKey(req, identifier) {
  ...
  const ip = Array.isArray(forwarded)
    ...
    : String(forwarded || req.socket?.remoteAddress || "unknown")
        .split(",")[0]
        .trim();
  return `${ip}:${normalizeIdentifier(identifier).toLowerCase()}`;
}
function allowLogin(req, identifier) {
  const key = getRateKey(req, identifier),
    now = Date.now();
  const current = (loginLimiter.get(key) || []).filter((ts) => now - ts < WINDOW_MS);
  if (current.length >= MAX_ATTEMPTS) return false;
  current.push(now);
  loginLimiter.set(key, current);
  return true;
}
const USER_FIELDS =
  "id,name,store_name,email,phone,region,district,avatar,bio,status,deleted_at,is_demo,created_at,password_hash,password";
async function findUser(supabase, identifier) {
  ...
  if (phone.length >= 7) {
    queries.push(supabase.from("users").select(USER_FIELDS).eq("phone", clean).limit(1));
    queries.push(supabase.from("users").select(USER_FIELDS).eq("phone", phone).limit(1));
  }
  ...
  return (
    candidates.find((user) => !user.deleted_at && String(user.status || "active").toLowerCase() !== "deleted") ||
    candidates[0]
  );
}
...
export default async function handler(req, res) {
  ...
  if (!supabase)
    return res.status(503).json({ success: false, error: "Authentication service is not configured on the server." });
  try {
    let body = req.body;
    ...
    if (!allowLogin(req, identifier))
      return res
        .status(429)
        .json({ success: false, error: "Terlalu banyak percobaan login. Silakan coba lagi beberapa menit lagi." });
    const user = await findUser(supabase, identifier);
    const genericFailure = () =>
      res.status(401).json({ success: false, error: "Nomor WhatsApp / Email / Nama Toko atau password tidak cocok." });
    if (!user) return genericFailure();
    const accountStatus = String(user.status || "active").toLowerCase();
    if (accountStatus === "deleted" || user.deleted_at)
      return res.status(403).json({ success: false, error: "Akun telah dinonaktifkan atau dihapus." });
    if (accountStatus === "suspended")
      return res.status(403).json({ success: false, error: "Akun sedang ditangguhkan oleh Admin." });
    let valid = false,
    ...
    if (needsLegacyMigration) {
      ...
      const { error: upgradeError } = await supabase
        .from("users")
        .update({ password_hash: upgradedHash, password: null, updated_at: new Date().toISOString() })
        .eq("id", user.id);
      if (upgradeError) console.error("[Auth Migration]", upgradeError.message);
    }
    const sessionToken = signUserSession(user);
    if (!sessionToken)
      return res.status(503).json({ success: false, error: "User session service is not configured on the server." });
    res.setHeader("Set-Cookie", userSessionCookie(sessionToken, req));
    return res.status(200).json({ success: true, user: publicUser(user) });
  } catch (error) {
    console.error("[Auth Login Error]", { name: error.name, message: error.message });
