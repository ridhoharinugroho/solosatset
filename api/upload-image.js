...
function validateImage(buffer, declaredType) {
  ...
  if (declaredType && declaredType !== detectedType)
    return { ok: false, error: "Tipe konten gambar tidak sesuai dengan isi file." };
...
export default async function handler(req, res) {
  ...
  if (!authenticatedUser?.sub && !authenticatedAdmin)
    return res.status(401).json({ success: false, error: "Authentication required." });

  const supabase = getAdminClient();
  if (!supabase)
    return res.status(503).json({ success: false, error: "Storage service is not configured on the server." });
  ...
  try {
    ...
    if (req.method === "DELETE" || body?.action === "delete") {
      if (!authenticatedAdmin)
        return res.status(403).json({ success: false, error: "Admin authentication required for storage deletion." });
    ...
    return res
      .status(200)
      .json({ success: true, publicUrl: publicUrlData.publicUrl, filePath: generatedFilePath, bucket: targetBucket });
  } catch (error) {
