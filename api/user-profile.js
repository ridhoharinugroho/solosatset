...
function publicFields() {
  return "id,name,store_name,email,phone,region,district,avatar,bio,status,deleted_at,is_demo,created_at";
}
...
export default async function handler(req, res) {
  ...
  if (!supabase)
    return res.status(503).json({ success: false, error: "Profile service is not configured on the server." });
  ...
  try {
    ...
    if (body?.email !== undefined) {
      ...
      if (!email) return res.status(400).json({ success: false, error: "Alamat email tidak valid." });
      updates.email = email;
    }
    ...
    if (!Object.keys(updates).length)
      return res.status(400).json({ success: false, error: "Tidak ada perubahan profil." });
    ...
    if (updates.phone) {
      ...
      if (error) throw error;
      if (data?.length)
        return res.status(409).json({ success: false, error: "Nomor WhatsApp sudah digunakan akun lain." });
    }
    ...
    if (String(error?.code || "") === "23505")
      return res.status(409).json({ success: false, error: "Email atau nomor WhatsApp sudah digunakan akun lain." });
