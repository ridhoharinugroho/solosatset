...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (!data || data.deleted_at || String(data.status || 'active').toLowerCase() === 'deleted') {
    ...
    if (updates.email || updates.phone) {
      const { data: conflicts, error: conflictError } = await supabase
        ...
        .or(`email.eq.${updates.email || '__no_email__'},phone.eq.${updates.phone || '__no_phone__'}`)
        .neq('id', userId)
        .limit(1);
      if (conflictError) throw conflictError;
      if (conflicts?.length) return res.status(409).json({ success: false, error: 'Email atau nomor WhatsApp sudah digunakan akun lain.' });
    }
