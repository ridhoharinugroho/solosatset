...
export default async function handler(req, res) {
  ...
  if (action === "logout" && method === "POST") {
    return json(
      ...
      200,
      { ok: true },
      { "Set-Cookie": `${SESSION_COOKIE}=; Max-Age=0; Path=/; HttpOnly; SameSite=Strict${secureFlag}` },
    );
  }
  ...
  return json(
    res,
    200,
    { ok: true, authenticated: true, user: { id: auth.user.id, username: auth.user.username, role: "admin" } },
    {
      "Set-Cookie": `${SESSION_COOKIE}=${encodeURIComponent(token)}; Max-Age=${SESSION_TTL_SECONDS}; Path=/; HttpOnly; SameSite=Strict${secureFlag}`,
    },
  );
}
