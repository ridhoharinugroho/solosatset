...
export function openCreateListingModal(editListingId = null) {
  if (!isUserLoggedIn()) {
    if (typeof window.openUserAuthModal === 'function') {
      window.openUserAuthModal('login', 'Silakan masuk atau daftar akun terlebih dahulu untuk memasang pasang iklan gratis!');
    } else {
...
export async function handleSaveListing() {
  ...
  const listingData = {
    ...
    createdAt: editingListingId ? (getListingById(editingListingId)?.createdAt || new Date().toISOString()) : new Date().toISOString(),
