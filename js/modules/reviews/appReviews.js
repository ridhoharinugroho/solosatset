import { getCurrentUser, getUserByReviewAuthor, getUserById } from "../../services/auth.js";
import {
  getAppReviews,
  getAppRatingStats,
  addAppReview,
...
export async function openAppReviewsModal() {
  ...
  if (currentUser) {
    ...
    const firstName =
      (currentUser.name || currentUser.storeName || currentUser.store_name || "Pengguna").trim().split(/\s+/)[0] ||
      "Pengguna";
...
export function selectAppReviewCategory(catId) {
  ...
  document.querySelectorAll(".picker-item-app-category").forEach((btn) => {
    ...
    if (isSelected) {
      btn.className =
        "picker-item-app-category w-full px-4 py-3 rounded-2xl border-2 border-rose-900 bg-rose-50/70 flex items-center justify-between gap-3 text-left transition-all cursor-pointer ring-2 ring-rose-900/20";
      ...
      btn.className =
        "picker-item-app-category w-full px-4 py-3 rounded-2xl border border-slate-200 hover:border-rose-300 bg-white hover:bg-slate-50 flex items-center justify-between gap-3 text-left transition-all cursor-pointer";
...
export function renderAppReviews() {
  ...
  reviews.forEach((rev) => {
    ...
    const catMeta = APP_REVIEW_CATEGORY_META[rev.category] || {
    ...
    const isOwner = Boolean(
      ...
      (rev.userId === currentUser.id ||
        ...
        (currentUser.email &&
          rev.userId &&
          String(rev.userId).toLowerCase() === String(currentUser.email).toLowerCase())),
    );
