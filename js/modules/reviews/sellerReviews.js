...
export function openSellerProfileModal(sellerIdOrObj) {
  ...
  const storeOrOwnerName =
    sellerUser?.storeName ||
    sellerUser?.name ||
    (typeof sellerIdOrObj === "object" ? sellerIdOrObj?.storeName || sellerIdOrObj?.name : "Toko");
  const avatarUrl =
    sellerUser?.avatar ||
    (typeof sellerIdOrObj === "object" ? sellerIdOrObj?.avatar : null) ||
  ...
  const distRaw = (sellerUser?.district || (typeof sellerIdOrObj === "object" ? sellerIdOrObj?.district : null) || "")
    .trim()
    .replace(/\.+$/, "")
    .replace(/^Kec\.?\s*/i, "");
  const districtName = distRaw
    ? distRaw
        .split(" ")
        .map((w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase())
        .join(" ")
    : "";
  const bioText =
    sellerUser?.bio || `Pusat jual beli barang amanah dan terpercaya di area ${regionName}. Pantau cocok bayar!`;
