...
export function initOverlayBubbleEvents() {
  ...
  document.getElementById("bubble-btn-bold")?.addEventListener("click", () => {
    ...
    const isBold =
      currentWeight === "700" || currentWeight === "800" || currentWeight === "900" || currentWeight === "bold";
      ...
      window.parent.postMessage(
        {
          ...
          customTexts:
            window.state?.customTexts || (typeof window.getCustomTexts === "function" ? window.getCustomTexts() : {}),
          siteSettings:
            window.state?.siteSettings ||
            (typeof window.getSiteSettings === "function" ? window.getSiteSettings() : {}),
        },
        "*",
      );
    } catch (_e) {}
...
export function saveElementStyle(key, prop, value) {
  ...
  if (!window.state.siteSettings)
    window.state.siteSettings = typeof window.getSiteSettings === "function" ? window.getSiteSettings() : {};
