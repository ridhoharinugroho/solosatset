...
import { trackUserInterest } from "../app/appAnalytics.js";
import { openModal, showToast } from "../../utils/modalRouter.js";
import { getUserById, isDemoUser, formatJoinedDate, isUserLoggedIn } from "../../services/auth.js";
import { openSellerProfileModal } from "../reviews/sellerReviews.js";
import { openUserAuthModal } from "../auth/authUI.js";
import { renderListings, applyDetailImageSettings } from "../products/listingsController.js";
...
export function openProductDetail(listingId, state) {
  ...
  if (thumbContainer) {
    if (listing.images && listing.images.length > 1) {
      ...
      listing.images.forEach((imgUrl, idx) => {
        thumbsHtml += `\n          <button\n            type="button"\n            data-img-index="${idx}"\n            class="detail-thumb-btn w-14 sm:w-16 aspect-square rounded-xl overflow-hidden border-2 transition-all ${0 === idx ? "border-rose-800 ring-2 ring-rose-300 scale-105" : "border-slate-300 opacity-70 hover:opacity-100"}"\n          >\n            <img src="${imgUrl}" alt="${listing.title} Foto ${idx + 1}" class="w-full h-full object-cover">\n          </button>\n        `;
      });
  ...
  if (priceEl) priceEl.textContent = formatRupiah(listing.price);
  ...
  const sellerId = listing.seller?.id,
    ...
    sellerRegionEl = document.getElementById("detail-seller-region"),
    sellerRegion = sellerRegionEl ? sellerRegionEl.querySelector("span") : null,
  ...
  if (waPreviewText) {
    const sellerDisp = listing.seller?.storeName || listing.seller?.name || "Penjual",
      locSnippet = listing.district ? `${regionName}, ${listing.district}` : regionName,
      buyerName = state.currentUser?.storeName || state.currentUser?.name || "Calon Pembeli",
...
function openShareModal(listing) {
  ...
  if (btnWaGroup) {
    ...
    btnWaGroup.onclick = () => showToast("Membuka WhatsApp untuk dibagikan ke Grup WA...", "success");
  }
...
function setupAdminToolbar(state) {
  ...
  if (objectFitSelect) objectFitSelect.value = currentSettings.objectFit || "cover";
  ...
  if (widthSlider) {
    widthSlider.oninput = (e) => {
      ...
      let ratioVal = 0.8;
      if ("aspect-square" === currentSettings.aspectRatio) ratioVal = 1;
  ...
  if (saveBtn) {
    saveBtn.onclick = (e) => {
      ...
      if (typeof window.broadcastStudioSync === "function") window.broadcastStudioSync();
