    ...
    async (event) => {
      ...
      if (form.id === "form-user-login") {
        ...
        if (!identifier || !password) {
          show(
            "login-error-alert",
            !identifier ? "Nomor WhatsApp, Email, atau Nama Pengguna harus diisi." : "Password harus diisi.",
          );
          return;
        }
      ...
      if (form.id === "form-user-register") {
        ...
        const payload = {
          ...
          email: String(document.getElementById("reg-input-email")?.value || "")
            .trim()
            .toLowerCase(),
      ...
      if (form.id === "forgot-password-form") {
        ...
        const email = String(document.getElementById("forgot-input-email")?.value || "")
          .trim()
          .toLowerCase();
        const button = form.querySelector('button[type="submit"]');
      ...
      if (form.id === "form-forgot-confirm") {
        ...
        const email =
          resetEmailInFlight ||
          String(document.getElementById("forgot-input-email")?.value || "")
            .trim()
            .toLowerCase();
        const otpCode = String(document.getElementById("forgot-input-code")?.value || "")
          .replace(/\D/g, "")
          .slice(0, 6);
        ...
        try {
          ...
          const verifyResult = await postJson("/api/auth-otp", {
            action: "verify",
