/**
 * Client authentication/session facade for Pusat Jual Beli Solo Raya.
 * Authentication authority lives on server endpoints under /api/*.
 * This module intentionally never stores, reads, syncs, or compares passwords.
 */
...
const DEFAULT_REGISTERED_USERS = [
  {
    id: 'user-102', name: 'Joko Supriyanto', storeName: 'Toko Pak Joko', email: 'joko.kra@gmail.com',
    phone: '085725012345', region: 'karanganyar', district: 'Jaten',
    avatar: 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?auto=format&fit=crop&w=150&q=80',
    bio: 'Pusat perabot rumah tangga & elektronik seken berkualitas Karanganyar.', status: 'active', isDemo: true,
    createdAt: '18 Juli 2026'
  },
  {
    id: 'user-103', name: 'Rian Kurniawan', storeName: 'Rian Gadget Kartasura', email: 'rian.gadget@gmail.com',
    phone: '089678123456', region: 'sukoharjo', district: 'Kartasura',
    avatar: 'https://images.unsplash.com/photo-1527980965255-d3b416303d12?auto=format&fit=crop&w=150&q=80',
    bio: 'Thrift & gadget bekas garansi personal area UMS Kartasura & Solo Baru.', status: 'active', isDemo: true,
    createdAt: '10 Juli 2026'
  },
  {
    id: 'user-104', name: 'Siti Aisyah', storeName: "Aisyah's Crafts Solo", email: 'aisyah.crafts@example.com',
    phone: '081234567890', region: 'solo', district: 'Mojosongo',
    avatar: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&w=150&q=80',
    bio: 'Handmade crafts, artwork, dan souvenir khas Solo. Fast WA response.', status: 'active', isDemo: true,
...
export function formatRegionTitle(rawRegion) {
  ...
  const reg = String(rawRegion).trim().toLowerCase();
  const map = {
...
export async function syncUsersFromCloud() {
  ...
  try {
    ...
    if (error || !Array.isArray(data)) return getRegisteredUsers();
    inMemoryRegisteredUsers = data.map(mapSupabaseUser).filter(Boolean);
    window.__registeredUsers = inMemoryRegisteredUsers;
    window.dispatchEvent(new CustomEvent('registeredUsersChanged', { detail: inMemoryRegisteredUsers }));
    return inMemoryRegisteredUsers;
  } catch (error) {
    console.warn('[Auth Users Sync]', error);
...
export function purgeLegacyDemoCache() {
  ...
  try {
    sessionStorage.removeItem(STORAGE_KEY_REGISTERED_USERS);
    localStorage.removeItem(STORAGE_KEY_REGISTERED_USERS);
    localStorage.removeItem(STORAGE_KEY_USER);
    localStorage.removeItem('barkas_user_session');
    localStorage.removeItem('solosatset_profile_cache');
    localStorage.removeItem('solosatset_seller_cache');
    localStorage.removeItem('solosatset_user_cache');
  } catch (error) {
...
export async function saveUserAvatarDirectly(userOrId, avatarUrl) {
  ...
  const targetEmail = typeof userOrId === 'object' ? userOrId?.email : current?.email;
  if (!targetId && !targetEmail) throw new Error('Pengguna tidak ditemukan.');

  const cleanAvatar = typeof avatarUrl === 'string' && avatarUrl.trim() ? avatarUrl.trim() : null;
  if (supabase) {
    const query = targetId
      ? supabase.from('users').update({ avatar: cleanAvatar, updated_at: new Date().toISOString() }).eq('id', targetId)
      : supabase.from('users').update({ avatar: cleanAvatar, updated_at: new Date().toISOString() }).eq('email', targetEmail.toLowerCase().trim());
    const { error } = await query;
    if (error) throw error;
  }

  if (current && current.id === targetId) setCurrentUser({ ...current, avatar: cleanAvatar });
  const index = inMemoryRegisteredUsers.findIndex((user) => user.id === targetId || (targetEmail && user.email === targetEmail));
  if (index >= 0) inMemoryRegisteredUsers[index] = { ...inMemoryRegisteredUsers[index], avatar: cleanAvatar };
...
export async function fetchFreshCurrentUserFromSupabase() {
  ...
  try {
    const query = current.id
      ? supabase.from('users').select('id,name,store_name,email,phone,region,district,avatar,bio,status,deleted_at,is_demo,created_at').eq('id', current.id).maybeSingle()
      : supabase.from('users').select('id,name,store_name,email,phone,region,district,avatar,bio,status,deleted_at,is_demo,created_at').eq('email', current.email).maybeSingle();
    const { data, error } = await query;
    if (!error && data) {
      const fresh = { ...current, ...mapSupabaseUser(data) };
...
export function findUserByIdentifier(identifier) {
  ...
  return getRegisteredUsers().find((user) => {
    ...
    if (user.storeName && user.storeName.toLowerCase() === normalized) return true;
    if (digits.length >= 7 && user.phone && user.phone.replace(/\D/g, '') === digits) return true;
    return false;
  }) || null;
}
...
export function setCurrentUser(user) {
  ...
  if (typeof window !== 'undefined') window.__currentUser = clean;
  notifySubscribers();
...
export async function registerUser({ name, storeName, phone, email, region, district, password, confirmPassword }) {
  const payload = await postJson('/api/auth-register', {
    name, storeName, phone, email, region, district, password, confirmPassword: confirmPassword ?? password
  });
  const user = stripCredentialFields(payload.user);
  setCurrentUser({ ...user, loggedInAt: new Date().toISOString() });
  return getCurrentUser();
...
export async function requestPasswordReset(email) {
  ...
  if (!cleanEmail || !cleanEmail.includes('@')) throw new Error('Masukkan alamat email valid yang terdaftar pada akun Anda.');
  const payload = await postJson('/api/password-reset', { action: 'request', email: cleanEmail });
...
export async function confirmPasswordReset(email, resetCode, newPassword) {
  const cleanEmail = String(email || '').trim().toLowerCase();
  const cleanCode = String(resetCode || '').replace(/\D/g, '').slice(0, 6);
  ...
  if (cleanPassword.length < 5) throw new Error('Password baru minimal 5 karakter.');

  const payload = await postJson('/api/password-reset', {
    action: 'reset', email: cleanEmail, otpCode: cleanCode, newPassword: cleanPassword
  });
  savePendingReset(null);
  return { success: true, email: payload.email || cleanEmail };
}
...
export async function updateProfile({ name, storeName, email, phone, region, district, bio, avatar }) {
  ...
  if (typeof finalAvatar === 'string' && finalAvatar.startsWith('data:')) {
    finalAvatar = await sbUploadAvatar(finalAvatar);
  }
  ...
  const clean = {
    ...
    storeName: storeName ? storeName.trim() : currentUser.storeName,
  ...
  if (supabase) {
    const { data, error } = await supabase
      ...
      .update({
        name: clean.name,
        store_name: clean.storeName || clean.name || null,
        email: clean.email || null,
        phone: clean.phone || null,
        region: clean.region || null,
        district: clean.district || null,
        bio: clean.bio || null,
        avatar: clean.avatar || null,
      ...
      .maybeSingle();
    if (error) throw error;
    if (data) Object.assign(clean, mapSupabaseUser(data));
  }

  setCurrentUser(clean);
  const index = inMemoryRegisteredUsers.findIndex((user) => user.id === currentUser.id);
  if (index >= 0) inMemoryRegisteredUsers[index] = stripCredentialFields(clean);
  window.dispatchEvent(new CustomEvent('registeredUsersChanged', { detail: inMemoryRegisteredUsers }));
  return getCurrentUser();
}
...
export async function removeUserAvatar(userId) {
  ...
  if (supabase) {
    const { error } = await supabase.from('users').update({ avatar: null, updated_at: new Date().toISOString() }).eq('id', targetId);
    if (error) throw error;
  }
  if (current) setCurrentUser({ ...current, avatar: null });
...
export async function logout() {
  ...
  try {
    ...
    [SESSION_KEY_USER_DATA, SESSION_KEY_USER_ID, STORAGE_KEY_USER, 'barkas_user_session', 'solosatset_profile_cache', 'solosatset_seller_cache', 'solosatset_user_cache']
      .forEach((key) => {
...
export function getUserById(userId) {
  ...
  return getRegisteredUsers().find((user) =>
    (user.id && String(user.id).toLowerCase() === target) ||
    (user.email && user.email.toLowerCase() === target)
  ) || null;
}
...
export function getUserByReviewAuthor(userId, authorName) {
  ...
  const target = String(authorName).replace(/\(.*?\)/g, '').trim().toLowerCase();
  return getRegisteredUsers().find((user) =>
...
if (typeof window !== 'undefined') {
  window.addEventListener('storage', (event) => {
    if ([STORAGE_KEY_USER, SESSION_KEY_USER_DATA, 'solosatset_logged_out'].includes(event.key)) notifySubscribers();
  });
}
