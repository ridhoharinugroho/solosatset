...
export async function confirmPasswordReset(email, resetCode, newPassword) { const cleanEmail = String(email || '').trim().toLowerCase(); const cleanCode = String(resetCode || '').replace(/\D/g, '').slice(0, 6); const cleanPassword = String(newPassword || ''); if (!cleanEmail || !cleanEmail.includes('@')) throw new Error('Email reset tidak valid.'); if (cleanCode.length !== 6) throw new Error('Masukkan 6 digit kode verifikasi yang Anda terima di email.'); if (cleanPassword.length < 8) throw new Error('Password baru minimal 8 karakter.'); const verifyPayload = await postJson('/api/auth-otp', { action: 'verify', email: cleanEmail, code: cleanCode }); const resetPayload = await postJson('/api/auth-otp', { action: 'reset_password', email: cleanEmail, verificationToken: verifyPayload.verificationToken, newPassword: cleanPassword }); savePendingReset(null); return { success: true, email: resetPayload.email || cleanEmail }; }
