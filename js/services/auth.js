...
export async function removeUserAvatar(userId) {
  ...
  if (!current || targetId !== String(current.id)) throw new Error("Pengguna tidak ditemukan.");
  const filePath = extractAvatarFilePath(current.avatar);
  if (filePath) await sbDeleteAvatar(filePath);
  const payload = await postJson("/api/user-profile", { avatar: null });
  const user = stripCredentialFields(payload.user);
  setCurrentUser({ ...current, ...user });
