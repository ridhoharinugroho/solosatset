...
export async function sbGetSiteSettings() {
  ...
  if (error) {
    console.error("[SupabaseDB] getSiteSettings:", error.message);
    return null;
  }
...
export async function sbSaveSiteSettings(settings) {
  ...
  if (error) {
    console.error("[SupabaseDB] saveSiteSettings:", error.message);
    return false;
  }
  return true;
}
...
export async function sbGetCustomTexts() {
  ...
  const { data, error } = await supabase.from("custom_texts").select("*").eq("id", "global").single();
  if (error) {
    console.error("[SupabaseDB] getCustomTexts:", error.message);
    return null;
  }
...
export async function sbSaveCustomTexts(texts) {
  ...
  const { error } = await supabase.from("custom_texts").upsert([payload], { onConflict: "id" });
  if (error) {
    console.error("[SupabaseDB] saveCustomTexts:", error.message);
    return false;
  }
  return true;
}
...
export async function sbGetSellerReviews(sellerId) {
  ...
  if (error) {
    console.error("[SupabaseDB] getSellerReviews:", error.message);
    return null;
  }
  return data;
}
...
export async function sbAddSellerReview(review) {
  ...
  try {
    const { data, error } = await supabase.from("seller_reviews").insert([review]).select().single();
    if (error) {
      console.error(
        "[SupabaseDB Error] Gagal menyimpan ulasan ke tabel seller_reviews:",
        error.message || error,
        error,
      );
      return null;
...
export async function sbDeleteSellerReview(reviewId) {
  ...
  if (error) {
    console.error("[SupabaseDB] deleteSellerReview:", error.message);
    return false;
  }
  return true;
}
...
export async function sbAddAppReview(review) {
  ...
  try {
    const { data, error } = await supabase.from("app_reviews").insert([review]).select().single();
    if (error) {
...
export async function sbDeleteAppReview(reviewId) {
  ...
  try {
    ...
    if (error) {
      console.error("[SupabaseDB] deleteAppReview error:", error.message);
      return false;
    }
    return true;
  } catch (e) {
...
export function sbSubscribeSettings(onChange) {
  ...
  const channel = supabase
    ...
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "custom_texts", filter: "id=eq.global" },
      (payload) => {
