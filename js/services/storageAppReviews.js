...
function safeBroadcastToCloud(type, data) {
  try {
    import('./cloudSync.js').then(({ broadcastToCloud }) => {
...
export function addAppReview({ rating, category, comment }) {
  ...
  const newReview = {
    ...
    userAvatar: currentUser.avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(currentUser.email || currentUser.id || fullUserName)}`,
