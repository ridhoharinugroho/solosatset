...
function openModal(){
  ...
  wrap.querySelector('[data-close-otp-password]').onclick=()=>wrap.remove();
  return wrap;
}
...
async function requestOtp(modal){
  ...
  modal.dataset.currentPassword=currentPassword;
...
async function complete(modal){
  ...
  const currentPassword=modal.dataset.currentPassword;
  ...
  const v=await callApi({action:'verify',purpose:'password_change',email,code});
  await callApi({action:'change_password',purpose:'password_change',email,verificationToken:v.verificationToken,currentPassword,newPassword});
  const user=getCurrentUser();
  if(user){const updated={...user,password:undefined};setCurrentUser(updated);}
  modal.remove();
...
function install(){
  ...
  document.addEventListener('click',async event=>{
    ...
    if(!(text.includes('ganti password')||text.includes('ubah password')||id.includes('change-password')||id.includes('ganti-password')))return;
    if(target.closest('#otp-password-change-modal'))return;
    event.preventDefault();event.stopPropagation();
    const user=getCurrentUser();
    if(!user?.email){window.dispatchEvent(new CustomEvent('auth:login-required'));return;}
    const modal=openModal();
    modal.querySelector('#otp-change-request').onclick=async()=>{try{modal.querySelector('#otp-change-error').textContent='';await requestOtp(modal);modal.querySelector('#otp-change-request').textContent='OTP Terkirim';modal.querySelector('#otp-change-request').disabled=true}catch(e){modal.querySelector('#otp-change-error').textContent=e.message}};
    modal.querySelector('#otp-change-verify').onclick=async()=>{try{modal.querySelector('#otp-change-error').textContent='';await complete(modal)}catch(e){modal.querySelector('#otp-change-error').textContent=e.message}};
  },true);
}
install();
