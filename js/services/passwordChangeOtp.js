...
function openModal(){
  ...
  wrap.querySelector('[data-close-otp-password]').onclick=()=>{pendingPasswordChange=null;wrap.remove()};
  return wrap;
}
...
async function complete(modal){
  ...
  await callApi({action:'change_password',purpose:'password_change',email:pending.email,verificationToken:v.verificationToken,currentPassword:pending.currentPassword,newPassword:pending.newPassword});
  const user=getCurrentUser();
  if(user){const updated={...user};delete updated.password;setCurrentUser(updated);}
